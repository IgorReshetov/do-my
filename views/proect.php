...
<div class="wrapper block-proect">
    ...
    <div class="row-proect" left="<?=$proect_left[1]?>" right="<?=$proect_right[1]?>">
        <div class="column1-proect column-proect">
            <div class="column-proect-text <?=($proect_left[1] == 1)?'display-block':''?>" row ="5" pos = "1">Кто в проекте больше всех заботится о пользователях?</div> 
        ...
        <div class="column3-proect column-proect">
            ...
            <div class="column-proect-text <?=($proect_left[1]+$proect_right[1] == 7)?'display-block':''?>"><b>Игорь:</b> разработчик back-end<br>/администратор хостинга сайта<br>/лидер команды.</div>
        </div>
    </div>
    ...
    <div class="row-proect" left="<?=$proect_left[2]?>" right="<?=$proect_right[2]?>">
        <div class="column1-proect column-proect">
            ...
            <div class="column-proect-text <?=($proect_left[2] == 3)?'display-block':''?>" row ="1" pos = "2">Кто в команде может не выполнить задачу спринта, но при этом выиграть войну?</div>
            <div class="column-proect-text <?=($proect_left[2] == 4)?'display-block':''?>" row ="0" pos = "0"><i>"Чтобы видеть дальше других, нужно стоять на плечах гигантов"</i></div>
        </div>
    ...
    <div class="row-proect" left="<?=$proect_left[3]?>" right="<?=$proect_right[3]?>">
        <div class="column1-proect column-proect">
            ...
            <div class="column-proect-text <?=($proect_left[3] == 3)?'display-block':''?>" row ="0" pos = "0"><i>"Чтобы видеть дальше других, нужно стоять на плечах гигантов"</i></div>
        </div>
        <div class="column2-proect column-proect column2-image3"><img src="views/images/icon/question_ring.svg" class="<?=($proect_right[3] > 0)?'opacity07':''?> <?=($proect_left[3]+$proect_right[3] == 5)?'display-none':''?>"></div>
        <div class="column3-proect column-proect">
    ...
    <div class="row-proect" left="<?=$proect_left[4]?>" right="<?=$proect_right[4]?>">
        <div class="column1-proect column-proect">
            ...
            <div class="column-proect-text <?=($proect_left[4] == 3)?'display-block':''?>" row ="0" pos = "0"><i>"Чтобы видеть дальше других, нужно стоять на плечах гигантов"</i></div>
        </div>
    ...
    <div class="row-proect" left="<?=$proect_left[6]?>" right="<?=$proect_right[6]?>">
        <div class="column1-proect column-proect">
            ...
            <div class="column-proect-text <?=($proect_left[6] == 3)?'display-block':''?>" row ="0" pos = "0"><i>"Чтобы видеть дальше других, нужно стоять на плечах гигантов"</i></div>
        </div>
