...
class Question 
{
    ...
    public function __construct($id_tree, $id_parent = false, $id_answer_branch = false) //создаем вопрос, если задан только первый аргумент возвращается первый вопрос дерева
    {
        ...
        if ($result->num_rows > 0) {
            ...
            $this->is_multi_answer = $question_data['is_multi_answer'];
        }
    }
    ...
    public static function getAll($id_tree) // нужно сосздать полный аналог таблицы spr_question_tree с присоединением таблицы spr_question
    {
        ...
        $query = "call getAllQuestion($id_tree)";
        $result = $mysqli->query($query);
